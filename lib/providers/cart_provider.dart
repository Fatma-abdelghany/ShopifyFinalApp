  ...
  void onIncreaseItemQuantityInCart(
    ...
    try {
      if (context.mounted) {
        ...
        int? itemIndex =
            cart.items?.indexWhere((element) => element.itemId == itemId);
        cart.items?.removeWhere((element) => element.itemId == itemId);

        updatedItem!.quantity = (updatedItem.quantity ?? 0) + 1;
        cart.items?.insert(itemIndex!, updatedItem);
      ...
      try {
        var selectedProductList =
            products.firstWhere((product) => product.id == item.productId);
             _total += (selectedProductList.price ?? 0) * (item.quantity ?? 0);

      } catch (e) {
        return null;
      } // var product =
      //     products.firstWhere((product) => product.id == item.productId,orElse: () => null);
      // _total += (selectedProductList.price ?? 0) * (item.quantity ?? 0);
    }
